...
def zero_shot_score_vcf(args, recordIndices, logits):
    """
    计算零样本分数并输出为 TSV 文件（不再输出 VCF）
    输出列：CHROM, POS, REF, ALT, PlantCAD2_score
    """
    logging.info("Calculating zero-shot scores and writing TSV output")
    nucleotides = ['A', 'C', 'G', 'T']

    # 构建记录索引到 logits 的映射
    idx_to_logit = {}
    for idx, logit in zip(recordIndices, logits):
        idx_to_logit[idx] = logit
    ...
    # 长度警告
    if total_records != len(recordIndices):
        logging.warning(
            f"VCF records: {total_records}, scored variants: {len(recordIndices)}. "
            "Unscored variants will have NaN scores."
        )

    # 输出 TSV 文件
    with open(args.output, 'w') as f_out:
        f_out.write("#CHROM\tPOS\tREF\tALT\tPlantCAD2_score\n")

        for rec_idx, record in enumerate(records):
            ref = record.REF.upper()
            alt_list = [str(alt).upper() for alt in record.ALT]
            if not alt_list:
                alt_list = ['.']

            probs = idx_to_logit.get(rec_idx)

            for alt in alt_list:
                if probs is not None and ref in nucleotides and alt in nucleotides:
                    ref_idx = nucleotides.index(ref)
                    alt_idx = nucleotides.index(alt)
                    ref_prob = probs[ref_idx]
                    alt_prob = probs[alt_idx]
                    if ref_prob > 0:
                        score = np.log(alt_prob / ref_prob)
                    else:
                        score = np.inf if alt_prob > 0 else np.nan
                else:
                    score = np.nan
                ...
                f_out.write(f"{record.CHROM}\t{record.POS}\t{ref}\t{alt}\t{score_str}\n")

    logging.info(f"Zero-shot scores written to {args.output}")
